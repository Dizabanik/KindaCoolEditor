...
#pragma region KCE_UNDO_REDO
int MAX_DELTA_NODES = 256;
...
int set_delta(char *text, int line, int column, bool add){
  current = (delta_node*)malloc(sizeof(delta_node));
  current->text = text;
  current->line = line;
...
int add_delta(char *text, int line, int column, bool add){
  if(current == NULL){
    return 1;
  }
  delta_node* cur = current->next;
  while (cur != NULL) {
      delta_node* temp = cur;
      cur = cur->next;
      if(temp->text != NULL){
  ...
  delta_node* new_node = (delta_node*)malloc(sizeof(delta_node));
  new_node->text = text;
  new_node->line = line;
  ...
  current = new_node;
  delta_count++;
  if(delta_count > MAX_DELTA_NODES){
    if(first != NULL){
      delta_node* temp = first->next;
      if(first->text != NULL){
        free(first->text);
      }
      free(first);
...
char currentFilenameCol[MAX_COLOR_CHARS] = "\x1B[32m";
char filenameCol[MAX_COLOR_CHARS] = "\x1B[37m";
...
void undo(){
  if(current != NULL){
    if(current->add == true){
      for(int i = 0; i < strlen(current->text); i++){
        editorRowInsertChar(&E.row[current->line], current->column, current->text[i]);
      }
...
void editorInsertChar(int c) {
  ...
  str[1] = '\0';
  if(delta_count > 0){
    add_delta(str, E.cy, E.cx, true);
  }
  else{
    set_delta(str, E.cy, E.cx, true);
  }
  if (E.cy == E.numrows) {
...
void editorCmdCallback(char *query, int key) {
  ...
  if(key == '\r' || key == '\x1b' || key == '\n'){
    ...
    #ifdef _WIN32
    strcat(buf, " > NUL 2>&1");
    system(buf); // replace "command" with your desired command
    #else
...
void editorCommandCallback(char *query, int key) {
  if(query[strlen(query)-1] == 'G' && strlen(query) > 1){
    char bufG[24] = "";
...
void editorDrawRows(struct abuf *ab) {
  ...
  struct stat statbuf;
  int longestDir = 20;
  char dNMame[20+(MAX_COLOR_CHARS*2)] = "";
      ...
      if(y == 0){
        ...
        if (dir != NULL) {
          if((ent = readdir (dir)) != NULL){
            ...
            strcpy(dNMame, "");
            if(strcmp(ent->d_name, E.filename) == 0){
              sprintf(dNMame, "%s%s%s", currentFilenameCol, ent->d_name, defaultCol);
            }
            else{
              sprintf(dNMame, "%s%s%s", filenameCol, ent->d_name, defaultCol);
            }
            ...
            abAppend(ab, dNMame, strlen(dNMame));
            abAppend(ab, " ", 1);
          }
          else{
            abAppend(ab, "                        ", 24);
